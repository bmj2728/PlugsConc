...
// GetPlugin retrieves a plugin from the catalog by its name in a thread-safe manner. Returns nil if not found.
...
// AddPlugin adds a plugin to the catalog, associating it with the specified name in a thread-safe manner.
...
// name is the identifier for the plugin.
...
// Cmd returns the command (`*exec.Cmd`) associated with the plugin's launch details.
func (p *PluginLaunchDetails) Cmd() *exec.Cmd {
	return p.cmd
}

// AllowedProtocols returns the list of protocols allowed for the plugin.
func (p *PluginLaunchDetails) AllowedProtocols() []plugin.Protocol {
